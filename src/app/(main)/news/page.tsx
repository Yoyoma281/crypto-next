...
function SentimentBar({ votes }: { votes: CPVotes }) {
  ...
  return (
    <div className="flex items-center gap-1.5 mt-1.5">
      <span className="text-[10px] font-semibold" style={{ color: "#4edea3" }}>
        {posPct}%
      </span>
      <div className="flex-1 h-1 rounded-full overflow-hidden bg-muted">
        <div
          className="h-full rounded-full"
          style={{ width: `${posPct}%`, background: "#4edea3" }}
        />
      </div>
      <span className="text-[10px] font-semibold" style={{ color: "#ffb3ad" }}>
        {100 - posPct}%
      </span>
    </div>
  );
