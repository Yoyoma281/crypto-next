...
export default function LeaderboardContent({ leaderboard }: { leaderboard: LeaderboardEntry[] }) {
  const { t } = useI18n();
  ...
  return (
    <div className="flex flex-col gap-6">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr
                ...
                style={{ borderBottom: '1px solid hsl(var(--border))' }}
              >
                <th className="text-left px-5 py-3 w-12">{t.leaderboard.rank}</th>
                <th className="text-left px-5 py-3">{t.leaderboard.trader}</th>
                <th className="text-right px-5 py-3">{t.leaderboard.portfolioValue}</th>
                <th className="text-right px-5 py-3">{t.leaderboard.vsStart}</th>
                <th className="text-right px-5 py-3 hidden md:table-cell">{t.leaderboard.share}</th>
              </tr>
            </thead>
            <tbody>
      ...
      <p className="text-xs text-muted-foreground text-center">{t.leaderboard.footer}</p>
    </div>
  );
}
